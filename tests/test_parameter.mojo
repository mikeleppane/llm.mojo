...
#
# The point of Parameter landing now (forward-only part) is that later parts add
# backward *methods* rather than restructuring every layer. So the invariants
# tested here are the ones the optimizer and backward pass will rely on: the grad
# exists, matches the value's shape, starts at zero, and zero_grad clears it.
