...
Uniform logits give loss = log(V). The gradient p - y sums to zero for a valid
distribution and one target. logsumexp is checked for numerical stability.
"""
...
def test_uniform_logits_loss_is_log_v() raises:
    """Uniform logits give loss log(V) (log(4) for V=4)."""
...
def test_grad_sums_to_zero() raises:
    """The gradient p - onehot sums to zero."""
...
def test_target_out_of_range_raises() raises:
    """The loss rejects a target outside [0, V)."""
    var logits = [0.0, 1.0]
...
def test_logsumexp_stable_under_large_values() raises:
    """The logsumexp of three 1000s is 1000 + log(3); a naive sum of exp(1000) would
    overflow."""
